...
import { UserInfoDataType } from '@/api/login_register/type'
...
export interface QrcodeStatusData {
    qrcode_status: number;
    uuid: string;
    socketId: string;
    userInfo?: UserInfoDataType;
}
socket.on('qrcode_status', (data: QrcodeStatusData) => {
    $bus.emit('qrcode_status', data)
})
...
// 加入当前通话的用户
socket.on('user_jion_telephone', (data) => {
    ipcRenderer.send('user_jion_telephone', data)
})

// 新加入的用户通知其他用户（用于界面展示）
socket.on('add_group_telephone', (data) => {
    ipcRenderer.send('add_group_telephone', data)
})

// 用户退出通话，通知其他用户
socket.on('close_telephone', (data) => {
    ipcRenderer.send('close-telephone-data', data)
})