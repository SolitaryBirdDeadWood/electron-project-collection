<template>
  <div class="message-content">
    ...
    <div class="chat-list">
      ...
      <div class="top-drag">
        <Search :showClose="showClose" @changeSearch="changeSearch" @getDataList="getSearchDataList">
          <el-tooltip class="box-item" effect="dark" content="新建群聊" placement="right-start">
            <div class="add-btn no-drag" @click="createGroup">
              <svg t="1689680839420" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                p-id="3330" width="20" height="20">
                <path
        ...
        <el-dialog v-model="createGroupDialog" width="45%" :show-close="false" style="background-color: #f2f2f2;">
          ...
          <div class="edit-content">
            <!-- 上传头像 -->
            <el-upload class="upload-demo" :limit="1" :show-file-list="false"
              :action="proxy.$baseUrl + reqUploadGroupPhoto" :on-error="handleUploadError"
      ...
      <div class="list" v-show="!showClose">
        ...
        <div class="apply-message-content">
          <div class="apply-message-friend" @click="handleShowApplyMessage(0)">好友通知
            <div class="right-wrap">
              <div class="dot" v-if="contactStore.friendContactNum != 0">{{ contactStore.friendContactNum > 99 ? '99+' :
                contactStore.friendContactNum }}</div>
              <el-icon>
                <ArrowRight />
              </el-icon>
            </div>
          </div>
          <div class="apply-message-group" @click="handleShowApplyMessage(1)">群通知
            <div class="right-wrap">
              <div class="dot" v-if="contactStore.groupContactNum != 0">{{ contactStore.groupContactNum > 99 ? '99+' :
                contactStore.groupContactNum }}</div>
              <el-icon>
                <ArrowRight />
              </el-icon>
            </div>
        ...
        <FriendOrGroupList :dataList="dataList" :showClose="showClose" :listType="listType"
      ...
      <div class="list search-list" v-show="showClose">
        <div class="friend-list" v-show="searchFriendsList.length">
          ...
          <FriendOrGroupList :dataList="searchFriendsList" :listType="0" :showClose="showClose"
            @changeRightComponent="changeRightComponent" />
        </div>
        <div class="group-list" v-show="searchGroupsList.length">
          <div class="list-title">群聊</div>
          <FriendOrGroupList :dataList="searchGroupsList" :listType="1" :showClose="showClose"
            @changeRightComponent="changeRightComponent" />
        </div>
    ...
    <!-- 默认组件 -->
    <div class="chat-content default" v-show="showRightComponent == 0">
      <DefaultContent />
    </div>
    <!-- 联系人详情组件 -->
...
import type { CreateGroupResponseData, FriendType, GroupType, GetFriendListResponseData, GetGroupListResponseData } from '@/api/contact/type'
// pinia
...
.message-content {
  ...
  .chat-list {
    ...
    .apply-message-content {
      width: 100%;
      border-bottom: var(--contact-apply-message-content-border);
...
:deep(.el-tabs__active-bar) {
  background-color: #4a89fc !important;
}
...
.edit-content {
  ...
  .user-pic {
    ...
    border-radius: 50%;
    overflow: hidden;
    border: var(--contact-user-pic-border);
    cursor: pointer;
    position: relative;
