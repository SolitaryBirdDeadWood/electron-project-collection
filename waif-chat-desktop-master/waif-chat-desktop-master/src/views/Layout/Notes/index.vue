<template>
      ...
      <div class="notes-item" v-for="(item, index) in notesList" :key="index">
        <div class="card" :style="`background-color: ${item.color};`">
          <div class="info">
            ...
            <el-popconfirm width="220" confirm-button-text="是的" cancel-button-text="取消" :icon="InfoFilled"
              icon-color="#626AEF" title="您确定要删除该便签吗?" @confirm="handleConfirm(item._id)"
  ...
  <el-drawer v-model="drawer" :direction="direction" size="40%">
    <template #default>
      <el-input v-model="notesText" type="textarea" :rows="6" placeholder="[歪fChat]请留下你想说的话..." maxlength="150"
        show-word-limit />
      <div class="notes-choose-color">
        <div class="color-title">颜色选择</div>
        <div class="color-list">
          <div class="color-item" v-for="(item, index) in notesColorArr" :key="index"
            :style="` background-color:${item};`" @click="changeNotesColor(index)">
